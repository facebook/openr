...
Config::checkBgpPeeringConfig() {
  ...
  if (isBgpPeeringEnabled() and config_.bgp_config_ref()) {
    const auto& srPolicyConfig = getSegmentRoutingPolicyConfig();
    ...
    for (const auto& peer : *config_.bgp_config_ref()->peers_ref()) {
      if (folly::IPAddress(peer.peer_addr_ref().value()).isLoopback() and
        ...
        if (not isSegmentRoutingEnabled()) {
          throw std::invalid_argument(
              "segment routing should be configured when BGP add_path is configured");
        }

        if (srPolicyConfig != std::nullopt and srPolicyConfig->size()) {
          throw std::invalid_argument(
              "segment routing (SR) policies should not be configured when BGP add_path is configured");
        }
      }
    }
  }
