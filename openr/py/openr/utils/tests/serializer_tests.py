...
class TestSerialization(unittest.TestCase):
    ...
    def test_configured_protocol_roundtrip(self) -> None:
        thrift_obj = openr_types.PrefixDatabase(thisNodeName="some node")
        raw_msg = serialize(thrift_obj, protocol=Consts.PROTO_FACTORY)
        recovered_obj = deserialize(
            openr_types.PrefixDatabase,
            raw_msg,
            protocol=Consts.PROTO_FACTORY,
        )
        self.assertEqual(thrift_obj, recovered_obj)
