...
@click.option(
    "--timeout",
    "-t",
    default=Consts.TIMEOUT_MS,
    type=int,
    help="Timeout for socket communication in ms",
)
@click.option(
    "--ports-config-file",
    "-f",
    default=None,
    type=str,
    help="JSON file for ports config",
)
@click.option("--color/--no-color", default=True, help="Enable coloring display")
@click.option("--verbose/--no-verbose", default=False, help="Print verbose information")
@click.pass_context
