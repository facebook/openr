...
TEST_F(FibTestFixtureWaitOnDecision, StaticRouteUpdates) {
  ...
  // Wait 2000ms to make sure Fib.client_ is initialized by keepAliveCheck(),
  // then send static MPLS routes to make sure normal code path of static routes
  // processing will be executed.
  evb.scheduleTimeout(
      std::chrono::milliseconds(scheduleAt += 2000), [&]() noexcept {
        // Make sure fib starts with clean route database
        std::vector<thrift::UnicastRoute> routes;
        std::vector<thrift::MplsRoute> mplsRoutes;
        mockFibHandler_->getRouteTableByClient(routes, kFibId);
        ...
        EXPECT_EQ(routes.size(), 0);
        mockFibHandler_->getMplsRouteTableByClient(mplsRoutes, kFibId);
        EXPECT_EQ(mplsRoutes.size(), 1); // Static MPLS route
...
/**
 * Ensure FIB could process route updates when FIB client connection is healthy
 * but not when unhealthy.
 * - Static MPLS routes are accepted when FIB client is healthy;
 * - FIB client flips to unhealthy.
 * - Followup RIB instance is unsuccessful.
 */
TEST_F(FibTestFixtureWaitOnDecision, FibClientFlipFromHealthyToUnhealthy) {
  // FIB client is initially CONFIGURED.
  ...
  evb.scheduleTimeout(
      std::chrono::milliseconds(scheduleAt += 2000), [&]() noexcept {
        // Add static MPLS routes into the queue.
        DecisionRouteUpdate staticRouteUpdate;
        staticRouteUpdate.mplsRoutesToUpdate.emplace_back(
            RibMplsEntry(label1, {mpls_path1_2_1}));
        staticRouteUpdate.mplsRoutesToDelete.emplace_back(label2);
        staticRouteUpdatesQueue.push(staticRouteUpdate);

        // Wait for MPLS route updates (delete was cleared).
        mockFibHandler_->waitForUpdateMplsRoutes();

        // Above Static routes are programmed.
        mockFibHandler_->getRouteTableByClient(routes, kFibId);
        EXPECT_EQ(routes.size(), 0);
        mockFibHandler_->getMplsRouteTableByClient(mplsRoutes, kFibId);
        EXPECT_EQ(mplsRoutes.size(), 1);

        // FIB client flips to UNINITIALIZED.
        mockFibHandler_->setSwitchRunState(
            thrift::SwitchRunState::UNINITIALIZED);
      });

  // Wait 2000ms to make sure Fib.client_ is set as UNINITIALIZED state.
  evb.scheduleTimeout(
      std::chrono::milliseconds(scheduleAt += 2000), [&]() noexcept {
        // Send normal RIB instance.
        DecisionRouteUpdate routeUpdate;
        routeUpdate.addRouteToUpdate(
            RibUnicastEntry(toIPNetwork(prefix1), {path1_2_1}));
        routeUpdate.unicastRoutesToDelete.emplace_back(toIPNetwork(prefix2));
  ...
  evb.scheduleTimeout(
      std::chrono::milliseconds(scheduleAt += 1000), [&]() noexcept {
        // Above RIB instance is not accepted.
        mockFibHandler_->getRouteTableByClient(routes, kFibId);
        EXPECT_EQ(routes.size(), 0);
        mockFibHandler_->getMplsRouteTableByClient(mplsRoutes, kFibId);
        EXPECT_EQ(mplsRoutes.size(), 1);
...
/**
 * Ensure FIB could not process route updates when FIB client connection is
 * unhealthy, but could process all backlogged route updates once healthy.
 * - Static MPLS routes arrives when FIB client is unhealthy;
 * - FIB client flips to healthy. It triggers full FIB sync including above
 *   static MPLS routes;
 * - Followup RIB instance is successful.
 */
TEST_F(FibTestFixtureWaitOnDecision, FibClientFlipFromUnhealthyToHealthy) {
  // FIB client is initially UNINITIALIZED.
  mockFibHandler_->setSwitchRunState(thrift::SwitchRunState::UNINITIALIZED);
  ...
  evb.scheduleTimeout(
      std::chrono::milliseconds(scheduleAt += 1000), [&]() noexcept {
        ...
        // FIB client flips to CONFIGURED.
        mockFibHandler_->setSwitchRunState(thrift::SwitchRunState::CONFIGURED);
