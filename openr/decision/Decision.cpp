  ...
  addFiberTask([q = std::move(kvStoreUpdatesQueue), this]() mutable noexcept {
    ...
    while (true) {
      ...
      try {
        ...
        XLOGF(
            FATAL,
            "Exception occured in Decision::processPublication - {}",
...
Decision::setRibPolicy(thrift::RibPolicy const& ribPolicyThrift) {
  ...
  runInEventBaseThread([this,
    ...
    if (durationLeft.count() <= 0) {
      XLOGF(
          ERR,
          "Ignoring RibPolicy update with new instance because of staleness. Validity {}ms",
          durationLeft.count());
      return;
    }
    ...
    XLOGF(
        INFO,
        "Updating RibPolicy with new instance. Validity {}ms",
        durationLeft.count());
  ...
  for (const auto& [area, areaPeerEvent] : event) {
    ...
    for (const auto& peerName : areaPeerEvent.peersToDel) {
      if (areaToPendingAdjacency_[area].erase(
        ...
        XLOGF(
            INFO,
            "[Initialization] No need to wait for dual directional "
...
Decision::saveRibPolicy() {
  ...
  ribPolicyFile.close();
  XLOGF(INFO, "Saved rib policy to {}", FLAGS_rib_policy_file);
}
...
Decision::readRibPolicy() {
  ...
  if (!ribPolicyFile.is_open()) {
    XLOGF(INFO, "Could not open rib policy file {}", FLAGS_rib_policy_file);
    return;
  }
  ...
  if (ttlDurationSec < 0) {
    XLOGF(
        INFO,
        "[Initialization] Skip loading expired rib policy file {}",
  ...
  try {
    ...
    if (key.find(Constants::kPrefixDbMarker.toString()) == 0) {
      ...
      if (*prefixDb.thisNodeName() == myNodeName_ && areaStack.size() > 0 &&
        ...
        XLOGF(
            DBG2,
            "Ignore self redistributed route reflection for prefix: {} area_stack: {}",
            key,
            folly::join(",", areaStack));
        return false;
      }
    ...
    XLOGF(
        ERR,
        "Failed to deserialize info for key {}. Exception: {}",
        key,
        folly::exceptionStr(e));
  }
  return false;
...
Decision::rebuildRoutes(std::string const& event) {
  ...
  XLOGF(
      INFO,
      "Decision: processing {} accumulated updates. {}",
      pendingUpdates_.getCount(),
      event);
  if (pendingUpdates_.perfEvents()) {
    if (auto expectedDuration = getDurationBetweenPerfEvents(
      ...
      XLOGF(
          DBG2,
          "Debounced {} events over {}ms.",
          pendingUpdates_.getCount(),
          expectedDuration->count());
    }
  }
...
Decision::unblockInitialRoutesBuild() {
  ...
  if (!initialRibPolicyRead_) {
    XLOGF(DBG1, "{}Have not read persisted Rib policy.", blockedLogPrefix);
    return false;
  }
  if (!initialPeersReceivedBaton_.ready()) {
    XLOGF(DBG1, "{}Initial peers not yet received.", blockedLogPrefix);
    return false;
  }
  if (!areaToPendingAdjacency_.empty()) {
    XLOGF(
