...
Decision::Decision(
  ...
  // RIB computation.
  if (config->isSegmentRoutingEnabled() and config->isBgpPeeringEnabled() and
      ...
      [p = std::move(p), filter = std::move(filter), this]() mutable {
        ...
        for (auto const& [area, linkState] : areaLinkStates_) {
          if (filter.selectAreas_ref()->empty() ||
              filter.selectAreas_ref()->count(area)) {
...
Decision::updateKeyInLsdb(
  ...
  try {
    if (key.find(Constants::kAdjDbMarker.toString()) == 0) {
      ...
      auto& nodeName = *adjacencyDb.thisNodeName_ref();
    ...
    if (key.find(Constants::kPrefixDbMarker.toString()) == 0) {
      ...
      // format.
      if (1 != prefixDb.prefixEntries_ref()->size()) {
        XLOG(ERR)
      ...
      auto const& entry = prefixDb.prefixEntries_ref()->front();
      auto const& areaStack = *entry.area_stack_ref();
      ...
      if (*prefixDb.thisNodeName_ref() == myNodeName_ && areaStack.size() > 0 &&
      ...
      PrefixKey prefixKey(
          *prefixDb.thisNodeName_ref(), toIPNetwork(*entry.prefix_ref()), area);

      pendingUpdates_.applyPrefixStateChange(
          *prefixDb.deletePrefix_ref()
