...
KvStore::processPublication() {
  ...
  auto deltaPub = mergeKeyValues(kvStore_, thriftPub.keyVals, filters_);
  ...
  floodPublication(std::move(deltaPub), folly::none);
}
...
KvStore::updatePublicationTtl(
  ...
  for (const auto& qE : ttlCountdownQueue_) {
    ...
    kv->second.ttl = timeLeft.count() - 1;
  }
}
...
KvStore::processRequest(
  ...
  case thrift::Command::KEY_SET: {
    ...
    auto thriftPub = mergeKeyValues(kvStore_, keyVals, filters_);
    updateTtlCountdownQueue(thriftPub);

    // publish the updated key-values to the peers
    floodPublication(std::move(thriftPub), thriftReq.keySetParams.originatorId);
...
KvStore::processSyncResponse() noexcept {
  ...
  auto deltaPub = mergeKeyValues(kvStore_, syncPub.keyVals, filters_);
  updateTtlCountdownQueue(deltaPub);
  ...
  floodPublication(std::move(deltaPub), originatorId);
...
KvStore::countdownTtl() {
  ...
  floodPublication(std::move(expiredKeysPub), folly::none);
}
...
KvStore::floodPublication(
  ...
  //
  // Update ttl on keys we are trying to advertise. Also do not publish keys
